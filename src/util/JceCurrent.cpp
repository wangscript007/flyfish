...
uint32_t JceCurrent::getUId() const
{
	return _uid;
}
...
void JceCurrent::setReportStat(bool bReport)
{
	_bReportStat = bReport;
}
...
void JceCurrent::initializeClose(const TC_EpollServer::tagRecvData &stRecvData)
{
	_ip = stRecvData.ip;
...
{
    _pServantHandle->sendResponse(_uid, string(buff, len), _ip, _port,_fd);
}
...
void JceCurrent::sendResponse(int iRet, const vector<char>& buffer, const map<string, string>& status, const string & sResultDesc)
{
    ...
    if (_request.iVersion != WUPVERSION && _request.iVersion != WUPVERSION2)
    {
		...
		LOGINFO("[TAF]JceCurrent::sendResponse :"
...
void JceCurrent::close()
{
    if (_pServantHandle)
    {
        _pServantHandle->close(_uid,_fd);
    }
}
...
void JceCurrent::reportToStat(const string& sObj)
{
    ...
    if(stat && stat->getStatPrx())
    {
		...
		//被调上报自己的set信息，set信息在setReportInfo设置，add by johnson
